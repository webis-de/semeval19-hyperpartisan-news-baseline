package de.aitools.ie.uima.analysis.segmentation;
...
public class InfexBASentenceSplitter extends JCasAnnotator_ImplBase{
	...
	throws ResourceInitializationException {
		...
		this.urlPrefixLexicon = this.loadLexicon(prefixPath, false, true);
		this.urlSuffixLexicon = this.loadLexicon(suffixPath, false, true);
		this.abbreviationLexicon = 
				this.loadLexicon(abbreviationPath, false, false);
		// Load other values
	...
	/**
	 * Loads the lexicon from the given absolute file path. The lexicon file is 
	 * supposed to contain one term per line. If one of the parameters 
	 * "toUpperCase" and "toLowerCase" is set, the terms are converted to upper 
	 * or lower case, respectively. In case both are set, the terms are 
	 * converted to upper case.
	 * 
	 * @param filePath The absolute file path
	 * @param toUpperCase Whether the loaded terms shall be stored upper case
	 * @param toLowerCase Whether the loaded terms shall be stored lower case
	 * @return The lexicon
	 */
	private Set<String> loadLexicon(String path, boolean toUpperCase, 
			boolean toLowerCase){
		Set<String> lexicon = new HashSet<String>();
    try (BufferedReader in = new BufferedReader(new InputStreamReader(
        ClassLoader.getSystemResourceAsStream(path), "UTF8"));) {
		    String str;
